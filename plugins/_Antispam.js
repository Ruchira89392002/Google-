...
export async function before(m) {
    ...
    if (!chats[m.chat].antiSpam || m.isBaileys || m.mtype === 'තහනම් පණිවිඩ සමූහයක්⭕' || m.mtype === 'ඔබගෙ පනිවිඩය තහවුරු කරමින් පවති 💬' || m.mtype === 'reactionMessage') {
        return;
    }
    ...
    if (timeDifference < 10000) {
        ...
        if (this.spam[m.sender].count >= 5) {
            ...
            setTimeout(() => {
                ...
                m.reply(`✅ *සන්සුන්ව සිටින්න තහවුරු ව්න තුරු*\nඉන් පසු ඔබට පනිවිඩ යැවිය හැකි👍.`);
            }, 5000);
            ...
            return m.reply(`❌ *කරුනාකර spam send නොකරන්න ${message}*\nරැදිසිටින්න⚠️️ ${Math.ceil((this.spam[m.sender].lastspam - now) / 1000)} තප්පර කිහිපයක්`);
        }
    } else {
